...
function InstallNoticeModal({ title, description, onClose }) {
  return (
    <div className="mypage__overlay mypage__overlay--center">
      <div
        className="mypage__install-modal"
        role="dialog"
        aria-modal="true"
        aria-labelledby="install-modal-title"
      >
        <span className="mypage__install-modal-mark" aria-hidden="true">
          PWA
        </span>
        ...
        <button
          type="button"
          className="mypage__install-modal-btn"
          onClick={onClose}
        >
          확인
        </button>
...
function InstallGuideView({ onBack }) {
  ...
  return (
    <div className="mypage mypage--install">
      <div className="mypage__header">
        <BackButton onClick={onBack} />
        <h1 className="mypage__header-title">앱 설치 방법</h1>
      </div>

      <div className="mypage__install-scroll">
        <section className="mypage__install-hero">
          <span className="mypage__install-hero-badge">PWA</span>
        ...
        <div className="mypage__install-guide-list">
          {INSTALL_GUIDE_SECTIONS.map((section) => (
            <article key={section.id} className="mypage__install-guide-card">
              <div className="mypage__install-guide-head">
                <span className="mypage__install-guide-badge">
                  {section.badge}
                </span>
                <div>
                  <h2>{section.title}</h2>
              ...
              <ol className="mypage__install-steps">
                {section.steps.map((step, index) => (
                  <li key={step} className="mypage__install-step">
                    <span
                      className="mypage__install-step-number"
                      aria-hidden="true"
                    >
                      {index + 1}
                    </span>
                    <span className="mypage__install-step-text">{step}</span>
                  </li>
                ))}
              </ol>

              <p className="mypage__install-tip">{section.tip}</p>
            </article>
          ))}
        </div>
      </div>

      <div className="mypage__install-action">
        <button
          type="button"
          className="mypage__install-action-btn"
...
export default function MyPage() {
  ...
  if (view === "install")
    return <InstallGuideView onBack={() => setView("main")} />;
  if (view === "library")
    return (
      <LibraryCategoryView
        books={libraryBooks}
